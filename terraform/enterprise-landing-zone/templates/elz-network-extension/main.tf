...
resource "oci_core_remote_peering_connection" "remote_peering_connection" {
  count            = (var.enable_vpn_or_fastconnect == "FASTCONNECT" || var.enable_drg_rpc) ? 1 : 0
...
*/
resource "oci_core_drg_attachment_management" "drg_rpc_attachment" {
  count              = (var.enable_vpn_or_fastconnect == "FASTCONNECT" && var.enable_fastconnect_on_environment) || var.enable_drg_rpc ? 1 : 0
  attachment_type    = "REMOTE_PEERING_CONNECTION"
  compartment_id     = var.network_compartment_id
  network_id         = oci_core_remote_peering_connection.remote_peering_connection[0].id
  drg_id             = var.drg_id
